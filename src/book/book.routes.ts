...
// CREATE
libroRoutes.post("/", requireAuth, async (req: Request, res: Response) => {
  const nuevo = await createLibroController(req.body);
  res.status(201).json(nuevo);
});

// UPDATE
libroRoutes.put("/:id", requireAuth, async (req: Request, res: Response) => {
...
// DELETE (soft)
libroRoutes.delete("/:id", requireAuth, async (req: Request, res: Response) => {
